...
window.triggerImport = () => {
  ...
  if (
    confirm("🚨 기존 데이터가 백업 파일 내용으로 덮어쓰기 됩니다. 계속할까요?")
  ) {
    input.click();
  }
};
...
window.importData = (event) => {
  ...
  reader.onload = async (e) => {
    ...
    try {
      ...
      window.taskTypes = (data.taskTypes || window.taskTypes || []).filter(
        Boolean,
      );
      window.coworkers = (data.coworkers || window.coworkers || []).filter(
        Boolean,
      );
      window.statuses = (data.statuses || window.statuses || []).filter(
        Boolean,
      );
      window.equipments = (data.equipments || window.equipments || []).filter(
        Boolean,
      );
      window.memoTags = (data.memoTags || window.memoTags || []).filter(
        Boolean,
      );
      ...
      try {
        ...
        console.warn(
          "복원 데이터가 커서 사진 이전 전 로컬 저장을 건너뜁니다.",
          storageError,
        );
      }
...
window.uploadToStorage = async function (dataUrl) {
  ...
  const response = await window.fetchWithTimeout(
    "https://work.sentig335.workers.dev/api/upload",
    {
      method: "POST",
      headers: {
  ...
  if (!result.ok || !result.url) {
    throw new Error("사진 업로드 응답에 URL이 없습니다.");
  }
      ...
      canvas.toBlob(
        async (blob) => {
          try {
            ...
            const r2Url = await window.uploadFileToR2(
              blob,
              file.name || "image.jpg",
            );
...
window.runStorageMigration = async () => {
  alert(
    "Firebase Storage 마이그레이션은 제거되었습니다. 현재는 로컬 + Cloudflare KV 동기화 방식입니다.",
  );
};
